...
The KNN test shows that a K of 37 neighbours prooved to yield a highest accuracy in this test-data. Keep in mind that the dataset only has 200 possible k’s so the boundary between the classes using linear method is not distinct at all. 
...
Gillard, Grønvold & Mähler,  *BIN315: Lab 5- Supervised learning*, Canvas/BIN315-2020/Week-5 (2020).
...
Code retrieved from Gillard, Grønvold & Mähler (2020)
```{r, eval=FALSE}  
...
Code inspired by Gillard, Grønvold & Mähler (2020)
```{r, results='hide',message=FALSE, warning=FALSE}
counter <- 0
data <- t(assay(feature_selected_data.norm))
accuracies <- NULL
...
while (counter < 1000) {
  ...
  y_predicted<-predict(rf.model, data[holdout,-5])
  df1<-data.frame(Orig=feature_selected_data.norm$classes2[holdout], 
                Pred=y_predicted)
